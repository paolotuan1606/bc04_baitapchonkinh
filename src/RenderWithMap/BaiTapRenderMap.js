...
export default class BaiTapRenderMap extends Component {
    ...
    renderMovie = () => {
        return this.arrMovie.map((item, index) => {
            return <div className='col-4 mt-5'>
                <div className="card text-left">
                    <div style={{ background: `url('${item.hinhAnh}') no-repeat center`, backgroundSize: 'contain', height: '300px' }}>
                        <img style={{ opacity: '0' }} src={item.hinhAnh} alt={item.hinhAnh} />
                    </div>
                    <div className="card-body">
                        <h4 style={{ height: 40 }} className="card-title">
                            {item.tenPhim.length > 23 ? <span>{item.tenPhim.substr(0, 23)}...</span> : <span>{item.tenPhim}</span>}
                        </h4>
                        <p style={{ height: 70 }} className="card-text" >
                            {item.moTa.length > 100 ? <span> {item.moTa.substr(0, 100)}...</span> : <span>{item.moTa}</span>}
                        </p>
                    </div>
                </div>
            </div>
    ...
    render() {
        return (
            <div className='container'>
                <h1 className='text-danger'>Danh sách phim</h1>
                <div className='row'>
                    {this.renderMovie()}
                </div>
            </div>
